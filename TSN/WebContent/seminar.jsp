...
<body class="page-header-fixed page-sidebar-closed-hide-logo "
						...
						<ul class="nav navbar-nav">
							...
							<li class="dropdown"><a href="#" data-toggle="dropdown"
								...
								<ul class="dropdown-menu">
									...
									<li><a
										href="<%=request.getContextPath()%>/ProfileController?flag=searchseminar">View
											Seminar</a></li>
	...
	<div class="page-container">
		...
		<div class="row" style="margin: 0 auto; width: 80%; float: none;">
			...
			<form id="seminar" name="seminar"
				action="<%=request.getContextPath()%>/ProfileController?flag=CreateSeminar"
				method="post">
				<!-- TASK TITLE -->
				<h1>
					<b>Create Seminar</b>
				...
				<div class="form-group col-sm-12">
					<div class="col-md-6">
						...
						<div class="custom-select-box">
							<select name="tsndesignation" id="tsndesignation"
								onchange="loadrole()"
					...
					<div class="col-md-6">
						...
						<select class="js-data-example-ajax" disabled name="tsndeliverby"
							id="tsndeliverby" style="border-radius: 15px;">
				...
				<c:forEach items="${sessionScope.passmatch}" var="i">
					<input type="hidden" name="tsnuserid" id="tsnuserid"
						value="${i.id}" />
				</c:forEach>
				<!-- TASK DUE DATE -->
				<div class="form-group col-sm-12">
					...
					<div class="col-sm-4">
						...
						<div class="col-sm-8">
							<span id="spdate" style="display: none; color: red">Enter
								valid date</span> <input class="form-control input-medium date-picker"
								size="16" type="text" name="tsndate" id="tsndate"
								onchange="date(this)" style="border-radius: 15px;" />
						</div>
				...
				<div class="form-group col-sm-12">
					...
					<div class="col-sm-12">
						...
						<div class="col-sm-4">
							<div class="custom-select-box">
								<select name="tsncountry" onchange="loadstate()" id="tsncountry"
									style="margin-bottom: 24px; border-radius: 15px !important;">
									<option value="" disabled selected>Select CountryName</option>
									<c:forEach items="${sessionScope.country}" var="j">
						...
						<div class="col-sm-4">
							<div class="custom-select-box">
								<select name="tsnstate" disabled onchange="loadcity()"
									id="tsnstate"
									style="margin-bottom: 24px; border-radius: 15px !important;">
									<option value="" disabled selected>Select StateName</option>
						...
						<div class="col-sm-4">
							<div class="custom-select-box">
								<select name="tsncity" disabled id="tsncity"
									style="margin-bottom: 24px; border-radius: 15px !important;">
									<option value="" disabled selected>Select CityName</option>
				...
				<div class="form-group">
					...
					<div id="fees" style="display: none;">
						<div class="col-sm-2" style="width: 12% !important;">
							<div class="custom-select-box">
								<select name="tsncurrency" id="tsncurrency"
									style="margin-bottom: 24px; border-top-left-radius: 15px; border-bottom-left-radius: 15px;">
				...
				<div class="col-sm-3" style="margin: 0 auto; float: inherit;">
					<input type="submit" name="submit" id="submit" value="CREATE"
						class="btn mb20 btn-medium btn-primary"
	...
	<script>
		...
		function loadrole() {
			...
			var role = document.getElementById("tsndesignation").value;
			// 			alert("123")
			...
			xmlhttp.onreadystatechange = function() {
				...
				if (xmlhttp.readyState == 4) {
					...
					for (i = 0; i < jsonObj.length; i++) {
						...
						createOption.value = jsonObj[i].userid;
						createOption.text = jsonObj[i].firstname;

						document.seminar.tsndeliverby.options.add(createOption);
						$("#tsndeliverby").removeAttr("disabled");
						// 						
					}
				}
			}
		...
		function removeAllrole() {
			var removeState = document.seminar.tsndeliverby.options.length;
			for (i = removeState; i > 0; i--) {
				document.seminar.tsndeliverby.remove(i);
			}
		}
	</script>
	<script>
		...
		function loadstate() {
			...
			var country = document.getElementById("tsncountry").value;
			// 			alert("123")
			...
			xmlhttp.onreadystatechange = function() {
				...
				if (xmlhttp.readyState == 4) {
					...
					for (i = 0; i < jsonObj.length; i++) {
						...
						document.seminar.tsnstate.options.add(createOption);
						$("#tsnstate").removeAttr("disabled");
						// 						
					}
				}
			}
			xmlhttp.open("get",
					"${pageContext.request.contextPath}/RoleController?flag=searchstate&country="
							+ country, true);
			xmlhttp.send();
		}
		/* Holds the status of the XMLHttpRequest. Changes from 0 to 4:
		...
		function removeAllstate() {
			var removeState = document.seminar.tsnstate.options.length;
			for (i = removeState; i > 0; i--) {
				document.seminar.tsnstate.remove(i);
			}
		}
	</script>
	<script>
		...
		function loadcity() {
			...
			var state = document.getElementById("tsnstate").value;
			// 			alert("123")
			// 			alert("Subject is :" + role);
			...
			xmlhttp.onreadystatechange = function() {
				...
				if (xmlhttp.readyState == 4) {
					...
					for (i = 0; i < jsonObj.length; i++) {
						...
						document.seminar.tsncity.options.add(createOption);
						$("#tsncity").removeAttr("disabled");
						// 						
					}
				}
			}
			xmlhttp.open("get",
					"${pageContext.request.contextPath}/RoleController?flag=searchcity&state="
							+ state, true);
			xmlhttp.send();
		}
		/* Holds the status of the XMLHttpRequest. Changes from 0 to 4:
		...
		function removeAllcity() {
			var removeState = document.seminar.tsncity.options.length;
			for (i = removeState; i > 0; i--) {
				document.seminar.tsncity.remove(i);
			}
		}
	</script>
	<script>
	...
	<script>
		...
		function date(x) {
			...
			curentdate.setDate(curentdate.getDate() + 1); //current date + 1 day
// 						alert(curentdate);
			var _MS_PER_DAY = 1000 * 60 * 60 * 24;
			var spdate = document.getElementById("spdate");
			if (d1.getDate() != curentdate.getDate()) {
				if (parseInt(Math.round(d1.getTime() - curentdate.getTime())
						/ _MS_PER_DAY) < 0) {
